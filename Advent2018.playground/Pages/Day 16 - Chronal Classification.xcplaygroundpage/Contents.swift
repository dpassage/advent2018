...
struct TestCase {
    ...
    init?(input: String) {
        ...
        self.before = before
        guard let operation = Operation(line: lines[1]) else { return nil }
    ...
    func validOperations() -> Int {
        var valid = 0
        for method in Device.allMethods {
            var device = Device(registers: before)
            method(&device, operation.a, operation.b, operation.c)
...
let day16cases = day16input.components(separatedBy: "\n\n").compactMap(TestCase.init)
